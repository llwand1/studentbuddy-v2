...
>   老板判决逐字「studentbuddy的题库功能也去删了,现在只要剩下内核和对战以及词条」（档 B，issue #32）。L0/L1 这条「出题前先问一次」的机制**整体保留**，
>   但本契约里凡写「聊天页**与题库页**两处出题按钮」「两页共用同一 hook」的地方，
>   现读只剩聊天页一处（`useAskStyle` 的持有者＝`AskStyleCard.tsx`＋`ChatView.tsx`）；
>   §6 未做项「判析（`/api/quiz/analyze/:id`）注入风格」自此**无对象**（该端点已删，见 `docs/QUIZ-WEAK-SPEC.md` 墓碑）。
>   ★ 另外本契约那句「下批任何再动 `quiz.ts` 的改动都必须先搬迁腾行（候选：把题库四函数搬去独立文件）」的
>   **行数红线理由已由本批替它做完了**——`saveQuiz`/`listQuiz`/`getQuiz`/`deleteQuiz` 四函数随整族下线全部删除，
>   `learning/quiz.ts` 现读 **352 行**（红线 400）。
