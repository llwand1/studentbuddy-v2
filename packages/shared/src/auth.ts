 ...
 *
 * ★★ 2026-09-22 起本常量为**休眠配置**：`register` 用途已从 `WIRED_PURPOSES` 摘出
 *   （注册不再要码，见 §2.7 的作废登记），故 `sendCode` 走不到这一格。
 *   ★ 不删它的理由与 `reset` 同一条：`IP_HOURLY_CAP` 按用途穷尽，且**将来重开邮箱验证时
 *   这道配套限流必须与消费端点同批回来**（当初就是这个耦合才没出事故）。
 *
...
/**
 * 同一 IP 每小时最多**注册**几个新账号（`auth/register-limit.ts`，契约 §2.7）。
 *
 * ★ 2026-09-22：注册改为免验证码直登之后**必须与它同批存在**的那道闸。
 *   理由：此前「注册要先输邮箱验证码」本身就是一道免费限流（要过 5 封/小时 + 5 封/小时/IP
 *   的发码闸，还烧 Resend 额度，脚本刷不动）；把码摘掉而不补这道计数，
 *   `POST /api/auth/register` 就变成**公开的无限建号机**。
 *   后果不是脏数据而已：每个新账号都带一份平台配额（v39 `platform_usage`，250 次/5h 走平台 key）
 *   ⇒ 一脚本能把我方免费 AI 额度**整池烧干**，症状是真人收到「繁忙，请稍后再试」。
 * ★ 阈值 5 沿用 `AUTH_CODE_MAX_PER_IP_REGISTER_HOUR` 的同一档判断：正常人一小时不会注册 5 次。
 * ⚠️ 已知代价与那条一模一样：**校园网 / 宿舍网共用出口 IP 时会误伤**（同一间教室第 6 个注册的人
 *   收到 429）。本机是单实例内存计数（同 §4.4/§4.5 的已知边界），挡在线刷、不挡分布式慢刷。
 */
export const AUTH_REGISTER_MAX_PER_IP_HOUR = 5;
...
export type AuthError =
  ...
  | 'MAIL_SEND_FAILED'
  /**
   * 注册过频（按 IP 计数）→ 429（2026-09-22 免验证码批，§2.7）。
   * ★ 与 `CODE_RATE_LIMITED` **分码不分档**：那个说"发信太频繁"，用户该等或改用密码；
   *   这个说"注册太频繁"，用户**今天可能压根不需要注册**（体验账号/GitHub 都在）。
   *   合成一码会让文案指错路——同 `CODE_INVALID`/`CODE_EXPIRED` 必须分开的同一条理由。
   */
