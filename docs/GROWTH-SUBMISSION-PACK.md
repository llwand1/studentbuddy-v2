...
> 版本：**v0.1.2** | 状态：[活跃] | 更新：2026-09-26（★ 本题库整族下线批：§2 一句话 A/C ＋ §3a/3b/3c 正文共 **7 处**在替已删功能背书（逐题正确率／薄弱点定位、概念图与知识图谱、每日总结），另有 **3 处**滞后账（§3a 出处那句「TERM_SPINE 五条」、§6.1 的侧栏项数与截图分档计数），本批 **10 处**全改成现查仍在的事实，总登记见本块下方那条 ★★；★ 前任 **v0.1.1**（2026-09-24 08:4x，批次 G-3）的表头**原样留在下一行未回改**——那行里有一个 G-3 留下的未闭合左括号（全文件净差 1，09-24 即如此，本批不碰他人行）。）
...
> ★★ **2026-09-26 全包扫假话改口（题库整族下线批，抄出去前请先读这条）**：包里有**十处**改口、其中**七处**是替**已经不存在的功能**背书（★ 与本页版本头那个「7 处＋3 处滞后账」同一个数、同一口径：七处＝§2 一句话 A／C、§3a 中文正文、§3b 那行「**析**」、§3c 英文那三行；另三处滞后账＝§3a 出处那句、§6.1 的侧栏项数与截图分档），本次全部改成仍在的事实——「逐题正确率／薄弱点定位」（本批随题库断线）、「概念图／知识图谱」「每日总结」（前两批随学习流与今日总结下线，★ 是本包**迟了两天的账**，不是本批删的）。⚠️ **同样的假话在产品对外面还有 10 处（6 个文件）＋1 处「演示窗在演一个已经不存在的动作」，本批一字未改**：① `src/app/landing-data.ts:87`＋`:89`「**学习流编排**」那一卡；② 同文件 `:162`＋`:164`「**知识图谱**」那一卡（那句还写着「薄弱环节一眼可见」，★ 本批又给它断了一层）；③ 同文件 `:283` `TERM_SPINE` 的「**连成图谱**：追问里抽出的新词自动连回它」；④ 同文件 `:314` `INTRO_THREE` 的「复习排期、**每日总结**各自都是一级功能」；⑤ `packages/web/index.html:7` 的 `<meta name="description">`（★ 搜索引擎摘要吃的就是这一行）；⑥ 同文件 `:9` 的 `og:description`（社交分享卡）；⑦ `src/seo/og-card.ts:120` 的 `alias` 同一句；⑧⑨ `src/seo/term-entries-a.ts:276` 与 `term-entries-b.ts:175` 两条 `productHint`（★ 现查：全仓 `概念图` 只剩这些**文案**，加上词条正文里讲「概念图是什么」的那几段——**没有任何幸存的功能模块**）；⑩ `docs/GROWTH-COMMUNITY-PACK.md:17` 的 **A 段主口播文案**「围绕它会自动长出复习排期、出题判分、**概念图和每日总结**」（★ 投稿包主用那一条，抄出去就是新造事实）。★★ 第 11 处**不是文案**＝`src/app/demo/registry.ts:67`＋`:113` 的 `GRAPH_FLOW`「知识图 · 追问长出关系」五帧，其中第 3／4／5 帧演的「追问抽出的词自动连回」「星长成树」「AI 连的边确认转实线」这个动作已于 09-25 删除（★ 现查逐帧定性：「向 AI 追问」这件功能**还活着**，在 `src/features/chat/TermCard.tsx:55`；死的是追问之后那步写 `knowledge_edge` 的 `linkFollowUpEdges`，见 `packages/server/src/chat/post-turn.ts:14` 那条注）⇒ ★ 这一处撞的是老板 09-21 那句「**实际效果和演示效果差别不要太大，别货不对板**」，而同文件 `PK_FLOW` 的头注还写着「每一帧都指得回一个真实屏态」——`GRAPH_FLOW` 这五帧此刻指不回。★ **本条注原先只登记了四处**（知识图谱卡／连成图谱／每日总结／OG 卡 alias），★ 收口复扫才把 `index.html` 两行 meta、两条 `productHint`、社区包 A 段与上面那处演示窗数进来——★ 病根＝当时只扫了「落地页文案常量」那一个文件，没扫 `index.html` 这个**最外层的对外表面**（★ 这条订正**不新增**上面那十处的账，改的是本注自己那句「四处」）。⇒ 那 11 处属**产品对外面**：改文要老板目检，改演示窗不是改字而是**换掉整个帧序列**（或删这支演示），且**都要发版才生效**，故本批未动、只在此登记；★ 一条判据分开跑——**发版后线上若这些句子仍在，测出来的是这 11 处，不许读成「题库没删净」**。
