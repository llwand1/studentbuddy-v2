 ...
 *
 * 为什么不进注册表（`chat/tools/`）：原理由是避让 `tools.ts` 并行在途改动（R1），该理由已随
 * 2026-09-19 S1 拆目录消失；现在仍未并回，是因为 P2 批按**纯重构**交付（工具行为一条不改），
 * 并回需要先收「当前清单状态归属」与注册表无状态签名的接缝，留后续批次。
 * 本工具走 flow.ts 的 exec 注入点接入（runToolCalls 支持自定义执行器），definition 由调用方拼接。
 *
