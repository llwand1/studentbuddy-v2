...
>   · ★ §2 第 1 条「情景题的每个评分点在 `quiz_stats` 里就是一道普通题」**本批作废**：逐题统计的写入口（`POST /api/quiz/stats/record`）已删，情景题从此**只判分不记账**（`learning/scenario.ts` 与 `shared/scenario.ts` 的注释同批回标）；⇒ 紧跟那句「正确率 / streak / 薄弱点分析 零新增接线」里，前两个名字共同的落点没了、第三个整族下线（issue #32 尾巴 ②④）。
