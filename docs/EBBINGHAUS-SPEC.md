...
- **不在范围内（下一批，方案待老板选）**：AI 督促（悬浮小窗对话）、~~自动安排到学习流~~（⚰️ 2026-09-25 宿主下线作废，见 §7）、跨设备同步。
...
- ~~复习与「学习流」「知识图」的打通（把逾期词条自动排进学习流）未做。~~ ⚰️ **2026-09-25 作废（批次 K）**：那两个功能已整族下线删除（迁移 v44 把 `flow_*`/`knowledge_*` 五张表 DROP）⇒ 这条"未做"**永远不会有做完的那天**，登记在此只为让下一次读到它的人不用再去查"学习流是什么、还欠不欠"。
