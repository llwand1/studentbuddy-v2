...
| 命令 | `npm run check`（tsc×3 + eslint + vitest **382 例 / 31 文件** + gates；2026-09-06 23:42 于 Node 22.23.2 全量实测绿，此前的账面数 339／378 已并入实测） |
...
| 优先级 | 测试项 | 说明 |
|--------|--------|------|
| ~~P1~~ ①②均已清偿 | **DOC-RAG 批欠的两条账**（2026-09-06 立，2026-09-07 结） | ① ~~`document.test.ts` 19 例与 `flow.test.ts` 18 例需在 **Node 22** 下跑~~ **已清偿**：2026-09-06 23:42 于 Node 22.23.2 跑全量 `npm run check` 绿（同时清偿认知进化批 `db.test.ts` v8 的 4 例）；② ~~**T9 真机端到端未跑**~~ **已于 2026-09-07 跑过**（老板批准花额度）：合成 77,291 字／492 段、冷事实埋第 423 段（86% 处）、`POST /api/doc` 回 `truncated:true` 当场证明走的是检索分支；Q1 关键词型四项事实全中且逐字引用原文、Q2 改写型三项中，跑完直连 SQL 净库复核残留 0。★ 但模型标的「资料段 N」是**本轮注入块内的相对序号、不是原文位置**，故「可溯源」只成立到「出自注入的第几段」（契约 §6.1 末行登记了这条边界，写回读时不得说成溯源完成）。本条属 AI 忠实度与体验类（§0.15），**「算不算过」的最终确认权仍在老板**，本行只登记仪器量得到的部分；同期仍欠 embedding 端点可用性（契约 §8.3）；pill 肉眼视觉已于同日出真页图（headless Chrome + CDP，before 6 张 + 打磨后 after 6 张），仪器层面已结、剩「算不算顺眼」归老板判 |
