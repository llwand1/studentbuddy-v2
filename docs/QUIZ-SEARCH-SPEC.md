...
> ⚠️ **2026-09-26 题库整族下线对本契约的影响（联网这条腿本身一行未改）**：
>   老板判决逐字「studentbuddy的题库功能也去删了,现在只要剩下内核和对战以及词条」（档 B，issue #32）。
>   · §2.2「三入口开关口径」自此**只剩两入口**：对话页（默认开）与 PK（硬编码开）——
>     原第一入口「题库页」随那页删除；★ 前端共用件 `components/OnlineToggle.tsx` 同批删除
>     （它其实早已零消费者：唯一调用方就是题库页那行）。
>   · §2.9「`RefList` 题库页（列表/练习两处）与对话页共用」⇒ 现读只剩 `ChatView.tsx` 一处挂载，组件保留。
>   · 「素材不是指令」「失败三层降级」「`refs` 一律删掉以免污染落库」三条**全部照旧生效**；
>     ★ 只是「污染**题库**」这个说法的对象变了：自此污染的是会话里的那一条题组消息。
