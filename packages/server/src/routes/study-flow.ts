...
studyFlowRouter.get('/defs', (req: Request, res: Response) => {
  res.json(listDefs(ownerIdOf(req)));
});

studyFlowRouter.post('/defs', (req: Request, res: Response) => {
  const r = createDef(req.body, ownerIdOf(req));
  if (!r.ok) {
...
studyFlowRouter.get('/defs/:id', (req: Request, res: Response) => {
  const def = getDef(req.params.id ?? '', ownerIdOf(req));
  if (!def) {
...
studyFlowRouter.put('/defs/:id', (req: Request, res: Response) => {
  const r = updateDef(req.params.id ?? '', req.body, ownerIdOf(req));
  if (!r.ok) {
    // 不存在 → 404；其余（校验不过）→ 400。域层不区分，故在此按「是否查得到」判一次
    const exists = getDef(req.params.id ?? '', ownerIdOf(req)) !== null;
...
studyFlowRouter.delete('/defs/:id', (req: Request, res: Response) => {
  const ok = removeDef(req.params.id ?? '', ownerIdOf(req));
  if (!ok) {
    res.status(404).json({ error: '学习流不存在' });
    return;
  }
...
studyFlowRouter.post('/defs/:id/clone', (req: Request, res: Response) => {
  ...
  const r = cloneDef(req.params.id ?? '', ownerIdOf(req), name);
  if (!r.ok) {
...
studyFlowRouter.get('/runs', (req: Request, res: Response) => {
  ...
  res.json(listRuns(ownerIdOf(req), Number.isFinite(limit) ? limit : 50));
});
...
studyFlowRouter.get('/runs/:id', (req: Request, res: Response) => {
  ...
  const run = getRun(id, true, ownerIdOf(req));
  if (!run) {
    res.status(404).json({ error: '运行实例不存在' });
    return;
  }
  res.json({ ...run, producedNodes: nodesOfRun(id, ownerIdOf(req)) });
});
...
studyFlowRouter.post('/runs/:id/advance', async (req: Request, res: Response) => {
  const r = await advanceRun(req.params.id ?? '', ownerIdOf(req));
  if (!r.ok) {
...
studyFlowRouter.post('/runs/:id/cancel', (req: Request, res: Response) => {
  ...
  const run = cancelRun(req.params.id ?? '', reason?.trim() || '用户主动终止', ownerIdOf(req));
  if (!run) {
...
studyFlowRouter.get('/graph/stats', (req: Request, res: Response) => {
  res.json(graphStats(ownerIdOf(req)));
});
...
studyFlowRouter.get('/graph/nodes', (req: Request, res: Response) => {
  ...
  const limit = Number(req.query.limit);
  res.json(listNodes(ownerIdOf(req), kind, Number.isFinite(limit) ? limit : 200));
});
...
studyFlowRouter.get('/graph/neighborhood/:nodeId', (req: Request, res: Response) => {
  ...
  const nb = neighborhood(req.params.nodeId ?? '', ownerIdOf(req), Number.isFinite(depth) ? depth : 2);
  if (!nb) {
...
studyFlowRouter.post('/graph/nodes', (req: Request, res: Response) => {
  ...
  res.status(201).json(ensureNode({ kind, refId: refId ?? null, refText: refText.trim(), ownerId: ownerIdOf(req) }));
});
...
studyFlowRouter.post('/graph/edges', (req: Request, res: Response) => {
  ...
  if (!getNode(fromNodeId, ownerIdOf(req)) || !getNode(toNodeId, ownerIdOf(req))) {
...
studyFlowRouter.delete('/graph/edges/:id', (req: Request, res: Response) => {
  const ok = removeEdge(req.params.id ?? '', ownerIdOf(req));
  if (!ok) {
...
studyFlowRouter.post('/graph/purge-derived', (req: Request, res: Response) => {
  res.json({ removed: purgeDerivedEdges(ownerIdOf(req)) });
});
