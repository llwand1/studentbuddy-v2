...
![tests](https://img.shields.io/badge/tests-207%20files%20%2F%202810%20cases-brightgreen)
...
**`npm run check` = tsc×3（shared/server/web）+ eslint + vitest + gates**，全绿才许提交。当前基线：**207 文件 / 2810 例**（2809 passed + 1 skipped + 0 failed，2026-09-25 游戏化 S1-a 批实测；上一读数 206/2799＝学习流·知识图下线批；**逐文件不变量见 [`docs/dev/test-plan.md`](docs/dev/test-plan.md) §3**，本格数字由 `node tools/metrics.mjs --tests` 产出）。
