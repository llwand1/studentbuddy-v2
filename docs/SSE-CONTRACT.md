...
| `block` | blockId/payload/done | 结构化内容块（演进③；M2 起启用，payload 见 shared/content-blocks）。~~**2026-09-06 登记 `kind:'verdict'`**：深度理解判定块（DEEP-UNDERSTANDING-SPEC §9.1），payload=`Verdict`（shared/domain，v1.1 含 `met`），`blockId='evo-<termId>-<ts>'`；由 flow 端 `[VERDICT]` 流式闸门吞掉正文后发射——判定块不上屏、不落 messages，「屏上文本==库内文本」铁律不破~~ ★ **2026-09-25 该 kind 摘除**：发射器（`POST /api/evolution`）从未落地、闸门零消费者，随深度理解族断线删除（issue #27，见墓碑与 §变更记录 2026-09-25 行）；这条登记从未产生过一帧线上事件 |
...
## 2.3 事件事实清单（新增 SSE 消费者 checklist）

> 立项依据（2026-09-20，健康检查批）：**B-007 / B-009 / B-010 三条同族**——根因全是「前端拿不可靠线索猜服务端事实」（seq 跨轮单调的假设、组件挂载时刻当轮起点、name+倒扫配对终态）。本节把该教训固化为**新增 SSE 消费者（或新增事件）的过闸清单**：`sse-events.ts` 是**类型**唯一事实源，本节是**「事实由谁携带 / 前端禁止推断什么」**的唯一事实源。新增事件必须先在下表登记「携带的事实」与「禁止的推断」两列，才允许实现。

### 2.3.1 既有事件的事实边界

| 事件 | 携带的事实（唯一键 / 基准 / 口径） | 前端禁止的推断（→ 教训） |
|------|-----------------------------------|--------------------------|
| `round-start` | `startedAt`＝服务端轮起点时刻；**恒为每轮缓冲第一帧** ⇒ 回放必带 | 禁止用组件挂载时刻起表（重挂即归零）（→ B-009） |
| `token` / `reasoning` | `content` 增量、按序追加；「流什么就存什么」⇒ 屏上==库内 | 禁止对同轮 token 做去重以外的本地加工/合并 |
| `block` | `blockId`＝块唯一键；同 id 整块替换、`done` 收口 | 禁止按 payload 形状猜块归属——认 `blockId` |
...
| `tool-confirm-*` | `requestId` 配对；`expiresAt` 只供倒计时显示，**裁决权在服务端定时器**（超时代答 `timeout`）；**批准 ≠ 已改**（`apply()` 在 allow 之后同轮执行） | 禁止前端本地判超时出手；禁止见 allow 就假定「已改完」（→ 与 choice「单一裁决者」同构） |
| `seq`（帧公共字段） | **换轮信号＝`seq===1`**（新一轮从 1 重计，`acceptSeq` 纯函数消费）；`since` 是客户端给的外部输入，服务端用前自证落在本轮区间内 | 禁止假设 seq 跨轮单调（→ B-007）；服务端禁止拿陈旧 `since` 直接当回放起点 |

### 2.3.2 新增 SSE 消费者 / 新增事件过闸七问

1. **频道键**：订阅键用 `shared` 的 channel 函数（`pkChannel()` / `coachChannel()`），不手抄前缀——三空间（`sessionId` / `pk:` / `coach:`）只靠前缀防串台。
2. **配对键唯一性**：凡按某字段配对事件，先问「这个字段在并发下同刻唯一吗」——不唯一就找真 id 上线路（`toolCallId` / `requestId` / `blockId`），**配不上另起、绝不回退猜测**（→ B-010）。
3. **可恢复性**：需要跨重挂/切会话存活的状态，必须有**可回放的帧承载**（`round-start` 模式）或上提到跨切换存活的层——没有第三条路（→ B-009）。
4. **换轮语义**：消费 `acceptSeq`（`seq===1` 即重置 since），不假设 seq 跨轮单调；本仓已撞过「第四个消费者才炸」的隐含假设（→ B-007）。
5. **计时事实源**：耗时/起点一律吃服务端字段（`startedAt` / `durationMs` / `thinkingMs`），本地 `Date.now()` 仅兜底——计时这类软状态坏得最安静。
6. **裁决权**：超时、判分、并发首答，裁决全在服务端；前端只做显示，不出手。
7. **负向清单**：正确答案、密钥/密文、内部配置细节——**永不进帧**（含错误回灌，→ B-006 教训同源）。
...
| 2026-09-25 | **订正上一条**（不改写它，按「沿革不回改」）：深度理解族断线下线（issue #27）⇒ `BlockKind` 摘掉 `'verdict'`、`ContentBlock` 的 payload 条件收窄同步摘分支、`DomainEvent` 摘掉 `evolution_levelup`、shared 四类型删除。**SSE 事件面零变化**：这两项登记自 2026-09-06 起从未发射过一次（发射器 `POST /api/evolution` 从未落地），所以前端与 `web/src/lib/sse` 一行未改。沿革见 `docs/DEEP-UNDERSTANDING-SPEC.md` 墓碑 |
