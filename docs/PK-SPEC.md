...
> 版本：v0.1.0 | 状态：[P0-1 登录已落地·房间/对战未开工] | 更新：2026-09-12
...
> P0-1 **仅登录链路已落地**（`pk_users` v10 迁移 / `POST /auth/login` / `GET /auth/me` / `UserAuthBox`，
> 提交 `e02d67a`，2026-09-09）；**房间与对战端点（`/rooms` 系列）、SSE `pk:` 频道、
> `PkRoomState`/`PkQuestion` 类型均未开工**。★ 头部状态此前长期写着「企划已定·未开工」，
> 与代码事实不符（登录 21:31 提交竟早于契约 21:11 定稿之后一天内落地），属状态漂移，2026-09-12 修正。
>
