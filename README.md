...
![tests](https://img.shields.io/badge/tests-196%20files%20%2F%202692%20cases-brightgreen)
...
- **析** —— 题卡当场判分 + 逐题解析、AI 主动督促与学习趋势（⚰️ 2026-09-26 题库整族下线：跨题的**逐题正确率累计**与**薄弱点定位**随族断线，老板判决逐字见 `CHANGELOG.md` 本批行与 `docs/QUIZ-WEAK-SPEC.md` 墓碑；尾巴挂 issue #32）
...
| **测试 + 机器强制门禁** | `npm run check` = tsc×3 + eslint + vitest + gates：单文件行数红线（server ≤400 / web ≤300）、禁 `any`、禁内联样式全部由脚本拦截，不靠自觉；交互层是**两层互补**——**23 个 `.test.tsx`**（★ 本批删的三件测试都是 `.ts`，这个 24 是上一批留下的旧抄，按 `find packages -name "*.test.tsx"` 实测改正）（jsdom 按文件 pragma 启用，锁交互逻辑）＋ **13 个真机探针脚本**（`tools/probes/*.mjs`，另有 1 件 `.mts`＝`og-card-cdp.mts`，合计 14 个文件；其中 8 个走 CDP 真点真渲染，锁 CSS 与真实浏览器行为。★ 2026-09-26 题库整族下线批删掉两件**专属**探针——`quiz-e2e-cdp.mjs`（点的是已下线的「题库」nav）、`weak-analysis-cdp.mjs`（打的是已断线的 `GET /api/quiz/analyze/:id`），15→13；先例＝批次 K 同批删 `flow-canvas-zoom-cdp.mjs`） | 基线 **196 文件 / 2692 例**（2691 passed＋1 skipped＋0 failed，2026-09-26 题库整族下线批（issue #32）后实测（★ 该批 −8 文件／−74 例＝删 8 件专属测试 72 例 ＋ 就地摘 4（`routes/scenario` 13→11、`features/quiz/scenario-view` 6→4）－ 搬入 2（`mixTipText` 两例随幸存函数搬进 `mix-report`）；★ 取证判据从「读 `quiz_bank`」搬到「读响应题组逐题 `source.kind` ＋ 会话 `[QUIZ]` 登记行」，另配两条「零落库」回归锁；★ **读数口径＝先 build 再测**）；上一读数 204/2766＝深度理解族断线批（issue #27）后实测（★ 该批 −3 文件／−35 例：删 `learning/verdict`／`learning/verdict-gate`／`domain` 三件测试，★ **零摘例、零改断言**＝其余 201 个测试文件一行未碰）；上一读数 207/2801＝移除「今日总结」＋「刷题笔记」批（issue #21）（★ 该批 −3 文件／−29 例：删 `learning/notes`／`routes/notes`／`note-format` 三件测试＋四处就地摘例；★ **读数口径＝先 build 再测**：本批首读在全新 worktree 里、`dist` 尚不存在，`src/seo/public-hygiene` 那两例 `it.skipIf(!HAS_DIST)` 当时是跳过的（2798 passed＋3 skipped）；补跑 `npm run build` 后重测＝下面这一读，两个读数**总量恒为 2801**，变的只是 passed/skipped 怎么分）；上一读数 210/2830＝构建产物卫生锁批（issue #8）（★ 先 build 再测：卫生锁里两例要读 `dist` 产物才跑）；上一读数 210/2826＝公开抓取覆盖锁批（issue #4）；上一读数 209/2820＝批次 I（主聊天 AI 图解能力批）随 PR #10 合并入 main；上一读数 208/2817＝搜索通道形状批；上一读数 207/2810＝游戏化 S1-a 批·再上一读数 206/2799＝学习流·知识图下线批·再上一读数 217/3004＝对战邀请批（`offer_pk_battle` → 迁移 v43 `pk_invites` → 聊天内联邀请卡）·上一档＝2026-09-24 归因批（`?ref=` → `X-SB-Ref` → 迁移 v42 `source` → `bySource`）后；★ 线上现况＝**已上线到 v0.2.132**（09-25 21:5x 三批合一＝联网搜索通道修复（bug-ledger B-019／issue #11）＋「今日总结」／「刷题笔记」断线下线（issue #21；★ 只断线、`daily_summaries`／`quiz_notes` 两张表仍在库里，DROP 归 v45 另批逐次点名）＋「连续学习 N 天」换算法（游戏化 S1-a，零迁移）；上一档＝09-25 11:44 三批合一＝渠道归因（迁移 v42）＋聊天内 AI 主动发起对战邀请（迁移 v43 `pk_invites`）＋学习流／知识图整族下线（迁移 v44 DROP 那七张表，按判决不留备份）；上一档 v0.2.118＝09-24 08:48 单批＝公开更新页＋Atom 订阅；**v0.2.113／114／115／116／117 五批** 09-23 23:5x 上线；**v0.2.109／110／111／112 四批**（中英切换／注册免验证码／SEO 静态页／目录页 URL 换正）09-23 09:56 上线。⚠️ 本句的导语在这里此前一直写着 `v0.2.112`——★ 那是上一轮的历史水位不是现况，而紧跟它的两句当场自纠到了 v0.2.118，**等于一句导语和它自己的下文打架**；现由 `node tools/metrics.mjs --check` 的「线上现况版本」一条与公开清洗表 `PUBLIC_RELEASES` 最高版本逐字对账，写旧即红。★ 发版逐次点名授权，本文件不预授权——那句「未发版」在它自己的历史上是对的，只是被今天改写了；线上判据实录见 `docs/SEO-SPEC.md` §6.2／§6.3），`metrics --tests` 实测口径；2691 passed + 1 skipped + 0 failed，全绿）；**逐文件不变量见 §3**（数字由 `node tools/metrics.mjs --tests` 产出，非手抄） |
...
### 练 + 析 · 出题（自建出题引擎）
- **`[QUIZ]` 结构化协议**：模型按字段清单 + 格式示例输出题组（题干 / 选项 / 答案索引 / 解析 / svg）整块随 SSE 交付——判分靠结构化作答索引，自动批改、不依赖模型逐次判卷（⚰️ 原句还写着「逐题可统计」：那条统计链 `POST /api/quiz/stats/record` → `quiz_stats` 已于 2026-09-26 断线，现零写者，尾巴见 issue #32）
...
- ⚰️ **题库与统计**（**2026-09-26 整族下线**，issue #32）：原「题目存 `quiz_bank`、答一题记一行 `quiz_stats`、按主题聚合出薄弱点」的三层——列表页 `QuizBankPage`、侧栏入口、`GET|DELETE /api/quiz/bank*` 四条、`POST /api/quiz/stats/record`、`GET /api/quiz/analyze/:id`（`learning/quiz-weak.ts`）全部断线（★ 出题入库那一步也断了：`POST /api/quiz/generate` 原先 `saveQuiz` 落一行 `quiz_bank`，本批起不再落库）；**留下的是出题引擎本身**（`/api/quiz/generate`、对战与聊天题卡在用）。两张表 `quiz_bank`／`quiz_stats` **本批不 DROP**，等一次独立的删除型迁移逐次点名；`quiz_bank` 自本批起唯一住户是**情景题套题**（`learning/scenario.ts` 读写），DROP 前必须先给它找新家——这三条尾巴都写在 issue #32 里，不在本批修
- ⚰️ **现场搜集真题**（v0.2.57 上线，**2026-09-26 随题库整族下线**）：原闭环是「题库页『搜集题目』→ 派生搜集词联网检索 → 受护栏抓页 → 模型从正文**逐字摘录**题目 → **verbatim 锚点锁**（题干锚点命不中原文即拒，宁漏真题不误收编题）→ preview/commit 两段人工确认才入库」。★ **断的是人工入口与落库，不是这套引擎**：被删的是 `CollectPanel`＋`collect-view` 两个前端件、以及 `routes/quiz.ts` 里的 `POST /api/quiz/collect/{preview,commit}` 两条端点；域层 `learning/collect.ts` 的 `collectQuiz` **仍在链上**——它是混合出题 `learning/quiz-blend.ts:147` 的真题侧（检索→抓页→逐字摘录→锚点锁一行没改），有 `learning/collect.test.ts` 钉着。★ 同批现查发现一条新尾巴：`normalizeCollectedQuiz`（commit 那步「不信任客户端」的复校验）自本批起**零生产调用方**（只有测试在钉）——已补进 issue #32，不在本批修。契约 `docs/RESOURCE-SPEC.md` 已整篇立墓碑（外部结果永不直接写库）（契约 [`RESOURCE-SPEC.md`](docs/RESOURCE-SPEC.md)）
...
### 忆 · 深度理解（⚰️ 2026-09-25 整族断线）
- 它从未成为功能：契约 v1.1 只落了三件地基件（`[VERDICT]` 流式闸门与解析、`shared` 类型登记、迁移 v8 的两张表＋`term_library` 三列），而**发判定的那条 `POST /api/evolution` 与前端渲染器从未落地** ⇒ 生产代码里没有一处写入，`evolution_session`／`evolution_event` 自建表起是空表（全仓仅 `storage/db.test.ts` 往里插过样例行）。
- ★ **本次按老板判决「只断线、DROP 另批」**（issue #27）：删掉零消费者的解析模块与类型登记，**页面与接口零变化**（本来就没有入口）。两张表与三列留在库里，DROP 与 `daily_summaries`／`quiz_notes` 同归后续迁移批**逐次点名**，本批不预授权。
- ⚠️ 三列暂时留着的硬理由：`term_delete_log` 的「撤销删除」要原样带回 `evo_level`／`best_level`／`evo_updated_at`——漏列**不报错、只静默回落 0**，`storage/term-delete-log.test.ts` 就锁着这一条。
- 沿革与设计取舍见 [`DEEP-UNDERSTANDING-SPEC.md`](docs/DEEP-UNDERSTANDING-SPEC.md)（⚰️ 墓碑）。
...
tools/probes    — 真机探针 14 个脚本文件（.mjs 13 ＋ .mts 1：CDP 真点 8 ＋ 能力/隔离量测 6；★ 2026-09-26 随题库整族下线删 `quiz-e2e-cdp.mjs`／`weak-analysis-cdp.mjs` 两件专属探针。原注「15 个（CDP 真点 10 + 能力/隔离量测 5）」是删前 `ls tools/probes/*.mjs` 的口径、且把 `.mts` 那件漏在括号算式外——本行按 `ls tools/probes/*.mjs *.mts` 现数重写）
```

**三包职责**：`@sb/shared` 只放契约与纯函数（前后端共用一份，不允许各写一套）；`@sb/server` 承载全部业务域；`@sb/web` 是 React 18 前端，**零第三方运行时依赖**。

**一级视图三个**（`App.tsx` 的 `View` 联合）：对话 / 词条 / 设置；对战是 `#/pk` 独立页，督促是常驻胶囊 + 抽屉。⚰️ 原为四个（多一项「题库」），2026-09-26 随整族下线删除（老板判决逐字见 `CHANGELOG.md` 本批行）——**出题这件能力没删**，它活在对话题卡与对战里。
...
- **外部结果永不直接写库**：⚰️ 原句是「现场搜集走 preview/commit 两段确认，服务端重算逐字锚点、不信模型上报」——那条人工入库通道 2026-09-26 随题库整族下线，**不变量本身反而更强了**：现在搜集产物（`collectQuiz` 的 candidates 只作混合出题的真题侧）**根本没有写库通道**，逐字锚点锁与 SSRF 护栏原样保留，代价与尾巴记在 issue #32
...
│  ├─ routes/               REST 分域路由（22 个域文件；全仓 REST 注册 120 条，2026-09-26 题库四组路由断线后现数）
...
**`npm run check` = tsc×3（shared/server/web）+ eslint + vitest + gates**，全绿才许提交。当前基线：**196 文件 / 2692 例**（2691 passed + 1 skipped + 0 failed，2026-09-26 题库整族下线批（issue #32）后实测（★ 该批 −8 文件／−74 例＝删 8 件专属测试 72 例＋就地摘 4－搬入 2，明细见 `docs/dev/test-plan.md` §3 顶注）；上一读数 204/2766＝深度理解族断线批（issue #27）（★ 该批 −3 文件／−35 例＝三份专属测试整体下线、零摘例零改断言）；上一读数 207/2801＝移除「今日总结」＋「刷题笔记」批（issue #21）（★ 该批 −3 文件／−29 例；上一读数 210/2830＝构建产物卫生锁批（issue #8）（★ 先 build 再测：卫生锁里两例要读 `dist` 产物才跑）；上一读数 210/2826＝公开抓取覆盖锁批（issue #4）；上一读数 209/2820＝批次 I（主聊天 AI 图解能力批）随 PR #10 合并入 main；上一读数 208/2817＝搜索通道形状批；上一读数 207/2810＝游戏化 S1-a 批；再上一 206/2799＝学习流·知识图下线批；**逐文件不变量见 [`docs/dev/test-plan.md`](docs/dev/test-plan.md) §3**，本格数字由 `node tools/metrics.mjs --tests` 产出）。
...
| `chat-composer-cdp.mjs` | 输入区「+」折叠菜单五项目 + 状态摘要「联网已开」挂没挂在触发器上 + 侧栏入口 | 需后端 18791 + vite 在 **5174**（端口写死）；★ 出题入口（`SB_PROBE_QUIZ=1` 才真跑，会花一次模型额度）现在是**真机唯一能验「菜单条目 → 转发到出题」接线**的手段——纯函数与 tsc 都测不到这条转发 |
...
> ⚰️ **2026-09-26 题库整族下线批（issue #32）随族删两件专属探针**：`quiz-e2e-cdp.mjs`（真点的是已下线的「题库」nav）、`weak-analysis-cdp.mjs`（打的是已断线的 `GET /api/quiz/analyze/:id`）——**两者的主语已不在**，留着就是能给假绿光的仪器。出题链路的真机核验改由 `chat-composer-cdp.mjs` 承担（`SB_PROBE_QUIZ=1`）。先例＝批次 K 同批删 `flow-canvas-zoom-cdp.mjs`；★ 那张表行是**本批顺手清掉的旧账**——文件 09-25 就随学习流族删了，表行没跟着走（现查 `git grep -n flow-canvas` 只剩历史与墓碑）。
...
| M2 练+析（出题引擎 / 题型配比 / SVG 配图 / ~~逐题统计~~） | ✅ 2026-09-04 → ⚰️ **2026-09-26 题库整族下线**：出题引擎／题型配比／SVG 配图**全保留**（对战与聊天题卡在用），砍掉的是题库页与「逐题统计＋薄弱点定位」这条派生链（尾巴挂 issue #32） |
...
| 现场搜集真题（逐字锚点锁 + 两段确认） | ✅ 2026-09-18 → ⚰️ **2026-09-26 随题库整族下线**：删的是题库页「搜集题目」面板与 `preview/commit` 两条端点；★ **引擎没删也没断线**——`learning/collect.ts` 的 `collectQuiz`（检索→抓页→逐字摘录→锚点锁）仍是混合出题的真题侧。沿革与判决留在 `docs/RESOURCE-SPEC.md` 墓碑 |
| 深度理解（判定链完整闭环） | 🔶 闸门件与落库表已就位，主链未接线 → ⚰️ **2026-09-25 整族断线**（零消费者的解析件与类型登记已删；两张表与 `term_library` 三列留库，DROP 另批逐次点名；沿革留在 `docs/DEEP-UNDERSTANDING-SPEC.md` 墓碑） |
...
| 全站搜索（FTS5 两族索引：消息 / 词条） | ✅ 已入库 2026-09-20（契约 `docs/FTS-SPEC.md`；迁移 v37；`GET /api/search`）★ 真机端到端待目检；★ 本行原写「三族：消息 / 词条 / 错题本」——第三类 `kind='note'` 早在 2026-09-25 随刷题笔记功能下线（`shared/fts.ts` 的 `FTS_KINDS` 现数只有两个，旧索引行由该清单当过滤器挡掉），表行是那条批的漏账，本批顺手改正。**P2 原计划加的 `'quiz'`（题库）也随 2026-09-26 题库整族下线作废**——没有题库可索引了 |
...
| [`DEEP-UNDERSTANDING-SPEC.md`](docs/DEEP-UNDERSTANDING-SPEC.md) | ⚰️ **墓碑**：深度理解契约沿革（`[VERDICT]` 闸门的跨 chunk 切断处理 / append-only 抗删链 / `best_level` 只增不减的取舍仍在）——★ 2026-09-25 整族断线（解析件与四类型已删，两表与 `term_library` 三列留库待 DROP 另批点名），文中"待评审／§14 WBS 待开工"字样均不再是事实 |
...
| [`QUIZ-WEAK-SPEC.md`](docs/QUIZ-WEAK-SPEC.md) | ⚰️ **墓碑**：薄弱点分析契约沿革（「AI 实时生成而非固定文本」的改判动机 / 降级不得冒充 AI 的 ADR-5 取舍仍在）——★ 2026-09-26 随题库整族下线（`GET /api/quiz/analyze/:id` 与 `learning/quiz-weak.ts` 已删，`quiz_stats` 零写者留库待 DROP 另批点名，尾巴挂 issue #32），文中"现役"字样均不再是事实 |
| [`RESOURCE-SPEC.md`](docs/RESOURCE-SPEC.md) | ⚰️ **墓碑**：现场搜集契约沿革（**verbatim 逐字锚点锁**「宁漏真题不误收编题」的取舍 / preview 永不落库的理由仍在，且引擎 `learning/collect.ts` 仍作混合出题的真题侧活着）——★ 2026-09-26 人工入口与两段确认端点随题库整族下线（尾巴挂 issue #32），文中「题库页『搜集题目』」等 UI 口径均不再是事实 |
