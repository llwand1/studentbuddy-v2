...
> **studentbuddy —— 把背知识做成一局一局的游戏。** 本地优先的 AI 学习产品：内容单元是**词条卡（Quizlet 式）**，驱动方式是**每天推着你练的那套游戏回路（多邻国式）**——连签、XP、即时判分、AI 主动约你打一局，全部跑在 **学 → 练 → 析 → 忆 → 反馈** 五环闭环上；同一套代码既可**单机本地运行**，也可作为**多用户 Web 服务**部署。★ 定位口径的由来与它的证伪条件登记在 [`docs/GROWTH-SPEC.md`](docs/GROWTH-SPEC.md) §7。
>
