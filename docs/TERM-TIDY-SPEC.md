...
>
> ⚠️ **v1.1 修订注记（2026-09-06，老板拍板⑦）**：三项中的「**直接应用不预览**」**已被 `docs/TOOL-ECOSYSTEM-SPEC.md` v1.2 就地取代**——`tidy_terms` 改走两阶段（`planTidy` 出方案 → 影响 >3 条时弹确认卡 → 才 `applyTidy` 落库），≤3 条仍保持直接应用。**另两点不变**：仍不做词条页按钮/自动触发，仍「只合并不删除概念」（本契约的删除指被并入行物理删除，概念保留在 aliases；**AI 主动删词条是另一个工具 `delete_terms`，权限与撤销语义见 TOOL-ECOSYSTEM-SPEC §5.1**）。本契约其余条款（方案形状、`normalizeTidyPlan` 校验、合并语义逐字段口径）不受影响，仍是实施基准。
...
不做：词条页按钮、自动触发、~~预览确认（均被否）~~（**v1.1 改**：预览确认已由 `TOOL-ECOSYSTEM-SPEC.md` v1.2 §4.6 按规模分档引入——影响 ≤3 条仍然不问、直接应用；>3 条必须先弹卡列清单。理由变化：当时无规模上限也无撤销，如今确认门与删除快照两道都在，「不打扰」与「不可逆」之间重排）。
