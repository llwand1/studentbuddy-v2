...
| `packages/server/src/pk/` + `packages/server/src/routes/pk.ts` | **PK 域（AI 出题微信双人对战，契约 `docs/PK-SPEC.md` v0.1.0）**。**P0-1 仅登录链路已落地**（提交 `e02d67a`）：`pk_users` 表（v10 迁移；`openid` 是 `UNIQUE` 且**按真 openid 设计**，P0 只写模拟值 `mock_<userId>`）+ `pk/auth.ts`（`normalizeNickname` trim 后 1~20 字／`getIdentity`／`loginOrRegister` 命中即找回原账号可改名、不命中即新建）+ 薄路由两端点 `POST /auth/login`（非法昵称 400 且一字不落库）、`GET /auth/me`（未知账号 404）。**房间与对战（`/rooms` 系列、计分状态机、计时器）、SSE `pk:` 频道、`PkRoomState`/`PkQuestion` 类型均未开工**——P1 换真微信网页授权只改 openid 写入方，响应结构（`PkIdentity`）与前端零改动 |
| `packages/shared/src/pk.ts` | PK 契约类型（P0-1 只有 `PkIdentity`；房间/对战/SSE 事件类型随后续批次在此追加登记） |
...
| M6 PK 微信答题（AI 出题双人对战，契约 `docs/PK-SPEC.md` v0.1.0） | 🔶 2026-09-09：**契约定稿**（与老板 grill 钉死六项——8 分钟固定时长局／P0 纯选择题／出题成功 +1、答对 +2、答错 −1、超时 −1、怠慢 −1 可累计且**分数可为负**／**全部计时在服务端**防作弊／**正确答案永不下发**给答题方／P0 局域网双人不做公网）。**P0-1 仅登录链路已落地**（`pk_users` v10 + `/auth/login`、`/auth/me` + `UserAuthBox`，提交 `e02d67a`）；**房间与对战、SSE `pk:` 频道未开工**。★ 本行 **2026-09-12 补登**（此前 PK 整条线在本文档无索引，属 §0.11 漂移），**「M6」编号亦为本次新增，待老板确认**。瓶颈不在代码：P1 真微信卡在**资质**（个人主体申请不到已认证服务号）＋公网 HTTPS/域名前置 |
