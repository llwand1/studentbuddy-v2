...
> 版本：v1.2 | 状态：[活跃] | 更新：2026-09-18（M2c 业务决策已定：用户自带 key + 平台免费额度**双通道**，见 §8.1.1）
...
#### 8.1.3 平台免费额度的三个硬约束（P0，按 GUIDE §0.5 风险评估）

★ **这是老板自掏腰包代付**，最坏情况是一个脚本刷出无上限的模型账单。三条缺一不可：

1. **模型白名单**：免费通道**只允许 1~2 个低成本模型**（2026-09-18 拍板「只免费指定便宜模型」）；想用贵模型必须自带 key。★ 不做白名单的话，"按 token 计量"形同虚设——**单次贵模型请求就能烧掉普通用户一天的额度**。
2. **每用户配额**：`token_usage` 按 `user_id` 聚合，达上限即拒（429 或降级到免费模型）。配额值**留 config**，不写死在代码里（§0.7 简洁优先 + 可调）。
3. **全局成本熔断**：跨用户的日/月总支出上限，达线即**关闭免费通道**（BYOK 通道不受影响）。★ 这一条是**唯一的兜底**——前两条都是"每用户"维度，**挡不住"注册一万个账号"**这类攻击。

⚠️ **未量化（诚实记账）**：免费额度的具体数值（送多少 token / 每天多少）与熔断阈值**均未定**，需老板给业务参数。在此之前 M2c 能开工的是**归属改造**（8.1.2）；**配额与熔断不能先写**——参数没定就写死的代码，改起来比重写贵。
...
★ **`app_settings` 的决策点**（原表未列，2026-09-18 补）：`key TEXT PRIMARY KEY, value TEXT` 当前语义是**全局一份**（出题配比 / 配图 / 回答方式偏好）。多用户后必须二选一：**① 全局默认**——所有用户共享一份，改一次全站生效，但**任何登录用户都能改**（同 `role_bindings` 的隐患）；**② 每用户一份**——主键改 `(owner_id, key)`，需重建表。★ 倾向 ②，理由同 8.1.2：`PUT /api/settings` 若是全局写口，**用户 A 改出题配比会改掉所有人的**。⬜ 待老板拍板，未拍板前不动。
