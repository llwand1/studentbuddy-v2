...
![tests](https://img.shields.io/badge/tests-62%20files%20%2F%20758%20cases-brightgreen)
...
| **758 例测试 + 机器强制门禁** | `npm run check` = tsc×3 + eslint + vitest + gates：单文件行数红线（server ≤400 / web ≤300）、禁 `any`、禁内联样式全部由脚本拦截，不靠自觉 | 一条命令本地/CI 复验；基线 **62 文件 / 758 例（757 passed + 1 skipped）**，2026-09-13 于 Node 22 全量 vitest 实测；**权威口径与逐文件对账见 `docs/dev/test-plan.md` §3**（本格数字仅为快照，随批次变动，勿据此判现状） |
...
**`npm run check` = tsc×3（shared/server/web）+ eslint + vitest + gates**，全绿才许提交。当前基线：**62 测试文件 / 758 用例**（2026-09-13 实测；**权威口径见 `docs/dev/test-plan.md` §3**，此处仅为快照）。
