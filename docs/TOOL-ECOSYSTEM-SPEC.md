...
2. **★ 内容闸门：只读网页正文，非网页必须如实拒绝（v1.4.3 补，真机实测驱动）**。两道闸——① `content-type` 白名单（`text/*` + 文本型 `application/*`；`application/pdf`／`image/*`／`application/octet-stream` 挡在门外，在 `res.text()` **之前**判，省掉把整个 PDF 读进内存）；② 二进制嗅探兜底（服务器**谎报**类型或不给类型时：解码后魔数 + 控制字符占比 > 2%）。**为什么非有不可**：`res.text()` 解二进制**不抛错**、`htmlToText` 又只剥标签 ⇒ 二进制会以「以下为网页正文」的名义回灌给模型。★ **真机实测（2026-09-20，`tools/probes/fetch-page-live.mjs`）**：修复前 pdf.js 样例 PDF → 回灌 **8021 字**（含 `%PDF-1.4`、850 个控制字符）、httpbin PNG → 回灌 **4736 字**乱码；修复后同一凭证复跑，PDF／PNG／SVG **三例全部如实拒绝、零字节二进制外泄**。**乱码当正文比「读不到」糟得多**——模型会把乱码当内容用。
