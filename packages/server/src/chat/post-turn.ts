 ...
 *
 * 三件事，**顺序不可换**：
 *   ① 抽词（`extractTerms`，一次 LLM 调用）→ ② 落词条库（`saveTerms`）
 *   → ③ **若是追问会话：连星型边**（`linkFollowUpEdges`）→ ④ 长期记忆压缩（`compactIfNeeded`）。
 *
 ...
 *
 * ★ 为什么 ③ 排在 ② 之后：边连的是"这个会话沉淀的词条"，词还没落库就没有可连的东西。
 *   排在 ④ 之前：压缩要打一次 LLM（配额有限），产物收口必须先做完（同原有注释的取舍）。
 *
 * ★ 为什么 ③ **不在 `if (items.length > 0)` 里面**：fork 会话的词条可能是**上一轮**落的
 *   （用户接着聊）。放在外面每次回复都跑一遍，靠库约束幂等；而它对**非追问会话是零成本空操作**
 *   （`linkFollowUpEdges` 第一件事就是查 `forked_term`，普通会话直接返回 0）。
 *
...
import { linkFollowUpEdges } from '../learning/follow-up-links.js';
...
export function afterTurn(input: {
  ...
  void extractTerms(`${text}\n\n${answer}`.slice(0, POST_TURN_TEXT_MAX), ownerId)
    .then((items) => {
      ...
      linkFollowUpEdges(sessionId, ownerId);
    })
