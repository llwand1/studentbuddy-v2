...
> 逐题给解析、AI 词条库 + 艾宾浩斯复习 + 跨会话记忆、事件总线驱动的学习反馈。
> ⚠️ **2026-09-26 订正**：这两行原先写「逐题统计薄弱点」——★ 那个统计面（`quiz_stats` 逐题记账）与薄弱点分析（`GET /api/quiz/analyze/:id`）随题库整族下线已删（issue #32），产品此刻做的是**当场判分＋逐题解析**，不再有跨题统计。
>
...
>
> 质量面：2690+ 个自动化用例进 CI（★ 现查 2026-09-26＝**2692 例／196 文件**，原写 2800+ 是题库断线批之前的水位）、机器强制的行数/样式/any 门禁、
> 8 个真实浏览器 CDP 探针（★ 现查订正：原写 15 把 13 件 `.mjs` 全算成了 CDP，真走 CDP 的是 7 件 `.mjs` ＋ 1 件 `.mts`）、一条命令可复跑的**零凭证全栈 E2E**（`npm run demo:e2e`）。
