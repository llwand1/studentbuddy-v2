...
| 10 | `learning/quiz-weak.ts:279` | `analyzer` | HTTP | ⚰️ **该行 2026-09-26 随文件删除失效**——本表是 M2c 开工实测的沿革快照，故原文保留；★ 连带后果：`MODEL_ROLES.analyzer` 自此**零调用方**，但 `routes/providers.ts:156` 仍把整张角色表回给前端 ⇒ 设置页还有一张卡可以绑一个永不被调用的角色（尾巴 ①，issue #32） |
...
| **M2d-1** | `daily_summaries` | `day` 单列 PK | ★ **重建** → `PK(owner_id, day)` | ✅ **已交付**（v0.2.62，迁移 v30）｜⚰️ **2026-09-25 失去宿主**：「今日总结」整页下线（issue #21），`todaySummary()` 与该表的读写面一并删除——**归主做完又没东西可管了**。表本批**未 DROP**（v45 单独一批），列与约束原样在位 |
...
| **M2d-3** | `quiz_bank` / `quiz_stats` / `quiz_notes` | 均以全局唯一 uuid 为主键 | **加列即可**（迁移 v33） | ✅ **已交付**（v0.2.67，2026-09-19）｜⚰️ 第三张 `quiz_notes` **2026-09-25 起没有读写了**（刷题笔记下线，issue #21；表等 v45 DROP），`quiz_bank`/`quiz_stats` 两列**照旧现役** |
| **M2d-3** | `flow_def` / `flow_run`(+step) | id PK / 随会话 | **加列即可**（迁移 v33；`flow_step`/`flow_edge` 刻意不加列——恒经 `def_id` 触达，归属随 `flow_def` 传递，同 `messages` 不加 user_id 口径） | ✅ **已交付**（v0.2.67，2026-09-19）⚰️ **2026-09-25 随宿主下线**（v44 DROP 这五张表，批次 K） |
| **M2d-3** | `knowledge_node` / `knowledge_edge` | id PK | **加列即可**（迁移 v33）——★ 本行同时是**已上线旧洞的收口**：改前这两张表无任何归属过滤，A 的知识图谱 B 能看见（M2d-1 普查发现） | ✅ **已交付**（v0.2.67，2026-09-19）⚰️ **2026-09-25 随宿主下线**（v44 DROP，那个"旧洞"也随之消失，批次 K） |
