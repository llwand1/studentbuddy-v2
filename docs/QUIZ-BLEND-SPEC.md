...
> ⚠️ **2026-09-26 题库整族下线对本契约的影响（合流逻辑本身一行未改，改的是它的落点）**：
>   老板判决逐字「studentbuddy的题库功能也去删了,现在只要剩下内核和对战以及词条」（档 B，issue #32）。
>   · 「混装成一个题组**入库**」这半句自此不成立：`quiz_bank` 不再被写入，题组只随 SSE 进会话。
>     ⇒ D5「`quiz_bank.source = 'blend'`，每题靠 `source.kind` 区分来源」失去存储位；
>     ★ 取证口径同批改到**读响应里每题的 `source.kind`**（`routes/quiz-blend.test.ts`、`chat/tools/generate-quiz.test.ts`），
>     并配两条「零落库」回归锁——所以 §「每题带来源」这条规范**仍可验、且更该验**。
>   · §8 对冲④「补题库页『剔除单题』入口（现只有 `deleteQuiz` 删整组）」自此**两头都没了**：
>     主语（题库页）与参照物（`deleteQuiz` 与 `DELETE /api/quiz/bank/:id/questions/:index`）同批删除
>     ⇒ 单题剔除自此**没有任何入口**；要恢复这件能力，先得给题组找一个能落库的地方（挂 issue #32 尾巴 ③）。
>   · 真题侧 `collectQuiz` 还在（见 `docs/RESOURCE-SPEC.md` 墓碑：死的是人工入口，不是引擎）⇒ **合流照旧出真题**；
>   · §6「题库列表徽标四分」随那页一起没了。
