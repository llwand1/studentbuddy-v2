...
> 版本：v1.2.1（变更记录见下表） | 状态：[WIP] | 更新：2026-09-19
...
>
> ⚠️ **v1.1 修订注记（2026-09-06，老板拍板⑦）**：三项中的「**直接应用不预览**」**已被 `docs/TOOL-ECOSYSTEM-SPEC.md` v1.2 就地取代**——`tidy_terms` 改走两阶段（`planTidy` 出方案 → 影响 >阈值 条时弹确认卡 → 才 `applyTidy` 落库），≤阈值 条仍保持直接应用。**★ v1.2.1 订正（2026-09-19，TOOL-ECOSYSTEM-SPEC v1.4 拍板⑮）**：弹卡阈值默认由立约时的 3 改定为 **5**（「点名合并 4 个同义词」属日常诉求不该弹卡），本节与 §不做 段里的「3 条」一律按「阈值（默认 5）」读。**另两点不变**：仍不做词条页按钮/自动触发，仍「只合并不删除概念」（本契约的删除指被并入行物理删除，概念保留在 aliases；**AI 主动删词条是另一个工具 `delete_terms`，权限与撤销语义见 TOOL-ECOSYSTEM-SPEC §5.1**）。本契约其余条款（方案形状、`normalizeTidyPlan` 校验、合并语义逐字段口径）不受影响，仍是实施基准。
...
| 2026-09-19 | v1.2.1 | 阈值措辞随 `TOOL-ECOSYSTEM-SPEC` v1.4 拍板⑮同步：弹卡阈值默认 3→**5**，本文两处「3 条」改按「阈值（默认 5）」读（头部 v1.1 注记与 §不做 段，纯口径订正不改承诺）。★ 顺带如实登记一处既有缺陷：v1.1 修订注记（2026-09-06 落，取代「直接应用不预览」）**当时只写在头部、未入本表**，本次不回填改写、仅此点名 |
...
- 设计范式：采用「**查重合并**」与「**别名挂靠**」两条成熟做法——同义词作为别名挂到主词条下，不丢概念（取范式，不取范式所属产品的包袱）。
...
不做：词条页按钮、自动触发、~~预览确认（均被否）~~（**v1.1 改**：预览确认已由 `TOOL-ECOSYSTEM-SPEC.md` v1.2 §4.6 按规模分档引入——影响 ≤阈值（默认 5，v1.4 拍板⑮订正，立约时 3）仍然不问、直接应用；>阈值 必须先弹卡列清单。理由变化：当时无规模上限也无撤销，如今确认门与删除快照两道都在，「不打扰」与「不可逆」之间重排）。
