...
const domainNames = () =>
  (getDb().prepare("SELECT name FROM term_domain WHERE owner_id = '' ORDER BY name").all() as Array<{ name: string }>).map((r) => r.name);

describe('learning/domains — 登记册（新建 / 改说明 / 统计）', () => {
  it('v19 迁移预置 general：新库即有默认领域，且零词条（count=0）', () => {
    expect(domainNames()).toContain('general');
    const stat = domainStats(null).domains.find((d) => d.domain === 'general');
    expect(stat?.count).toBe(0);
  });

  it('新建领域**允许零词条**，并立刻出现在统计里（v19 前做不到：没词条就不存在领域）', () => {
    const { row, created } = createDomain('Math', '高数与线代', null);
    expect(created).toBe(true);
    expect(row.name).toBe('math'); // 名称规范化：小写
    expect(row.note).toBe('高数与线代');
    expect(row.count).toBe(0);

    const stat = domainStats(null).domains.find((d) => d.domain === 'math');
    expect(stat?.count).toBe(0);
    expect(stat?.note).toBe('高数与线代');
  });

  it('新建已存在的领域：created=false，且**不覆盖**既有说明（幂等，点两次不报错）', () => {
    createDomain('cs', '原说明', null);
    const again = createDomain('CS', '新说明', null);
    expect(again.created).toBe(false);
    expect(again.row.note).toBe('原说明');
  });

  it('空领域名 → 400', () => {
    expect(catchDomainError(() => createDomain('   ', '', null)).status).toBe(400);
  });

  it('改说明只动 note（领域名不变）；领域不存在 → null', () => {
    createDomain('bio', '旧', null);
    const row = updateDomain('bio', '新', null);
    expect(row?.note).toBe('新');
    expect(row?.name).toBe('bio');
    expect(updateDomain('nope', 'x', null)).toBeNull();
  });
...
describe('learning/domains — 改名（词条随迁 + 撞域并入）', () => {
  it('**空领域改名**：只动登记册，moved=0（v19 前这条路径根本走不通）', () => {
    createDomain('mathx', '', null);
    const r = renameDomainEntry('mathx', 'math', null);
  ...
  it('有词条的领域改名：词条随迁，count 跟着走', () => {
    saveTerms([{ term: 'closure', definition: '闭包', domain: 'cs', importance: 0.8 }], null, null);
    const r = renameDomainEntry('cs', '计算机', null);
    expect(r.moved).toBe(1);
    expect(listTerms('计算机', undefined, null)).toHaveLength(1);
    expect(listTerms('cs', undefined, null)).toHaveLength(0);
    expect(domainStats(null).domains.find((d) => d.domain === '计算机')?.count).toBe(1);
  });
  ...
  it('目标领域**已存在** ⇒ 两域合一（merged=true，旧名从登记册移除，词条汇入）', () => {
    ...
    const r = renameDomainEntry('数学', 'math', null);
    expect(r.merged).toBe(true);
    expect(r.moved).toBe(1);
    expect(domainNames()).not.toContain('数学');
    expect(domainStats(null).domains.find((d) => d.domain === 'math')?.count).toBe(2);
  });
  ...
  it('目标领域已有**同名词条**：并入而不是撞 UNIQUE(term, domain) 抛错', () => {
    ...
    expect(() => renameDomainEntry('english', 'math', null)).not.toThrow();
    const merged = listTerms('math', undefined, null);
  ...
  it('领域不存在 → 404；新旧同名 → 400', () => {
    expect(catchDomainError(() => renameDomainEntry('ghost', 'cs', null)).status).toBe(404);
    createDomain('cs', '', null);
    expect(catchDomainError(() => renameDomainEntry('cs', 'cs', null)).status).toBe(400);
  });
});
...
describe('learning/domains — 删除（词条迁 general，一条不删）', () => {
  it('删除领域：词条**迁入 general 而非删除**，登记册移除旧名', () => {
    ...
    const r = removeDomain('temp', null);
    expect(r.moved).toBe(2);
    expect(r.target).toBe('general');
    expect(listTerms(undefined, undefined, null)).toHaveLength(2); // 词条一条未丢
    expect(listTerms('general', undefined, null)).toHaveLength(2);
    expect(domainNames()).not.toContain('temp');
  });

  it('general 是默认领域，拒绝删除（409）——它是迁词条的终点', () => {
    expect(catchDomainError(() => removeDomain('general', null)).status).toBe(409);
    expect(domainNames()).toContain('general');
  });

  it('领域不存在 → 404', () => {
    expect(catchDomainError(() => removeDomain('ghost', null)).status).toBe(404);
  });
  ...
  it('general 里已有同名词条时删域：并入不炸约束', () => {
    ...
    expect(() => removeDomain('other', null)).not.toThrow();
    expect(listTerms('general', undefined, null)).toHaveLength(1);
  });
});

describe('learning/domains — 写入侧登记 + 孤儿兜底（v19 不变式）', () => {
  it('saveOneTerm 落新领域 → 登记册自动出现（无需人工同步）', () => {
    saveOneTerm('牛顿', '力学家', 'physics', null);
    expect(domainNames()).toContain('physics');
  });

  it('saveTerms 落新领域 → 同样自动登记（AI 抽取吐新领域名的路径）', () => {
    saveTerms([{ term: '熵', definition: '混乱度', domain: 'thermo', importance: 0.6 }], null, null);
    expect(domainNames()).toContain('thermo');
    expect(domainStats(null).domains.find((d) => d.domain === 'thermo')?.count).toBe(1);
  });
  ...
  it('孤儿域兜底：绕过写入侧直插词条（登记册缺失）时，统计仍能列出该域，不让词条"隐身"', () => {
    ...
    const stat = domainStats(null).domains.find((d) => d.domain === 'orphan');
...
describe('learning/domains — 领域总提及数 + 偏好领域（契约 §2）', () => {
  ...
  it('preferred 按提及数降序；**同提及数时按词条数降序**（4=4 时 math 的 2 条压过 english 的 1 条）', () => {
    ...
    expect(domainStats(null).preferred.map((p) => p.domain)).toEqual(['math', 'english']);
  });
  ...
  it('preferred 是**全序**：提及数与词条数都相同时按领域名升序（同一份数据每次顺序逐字相同）', () => {
    ...
    expect(domainStats(null).preferred.map((p) => p.domain)).toEqual(['alpha2', 'zeta']);
  });
  ...
  it('零提及领域**不进** preferred，但仍照旧出现在 domains（mentionCount=0，不隐藏）', () => {
    ...
    createDomain('empty', '', null); // 连词条都没有的空领域
    mention('alpha', 2);
...
describe('learning/domains — 复习范围计数（v28）', () => {
  const find = (name: string) => domainStats(null).domains.find((d) => d.domain === name);
  const scopeTerm = (term: string, enabled: boolean) =>
    getDb()
      .prepare("UPDATE term_library SET review_enabled = ? WHERE term = ? AND owner_id = ''")
      .run(enabled ? 1 : 0, term);

  it('默认全不选：reviewEnabled=false 且 reviewCount=0（空领域也是 0）', () => {
    createDomain('empty-one', '', null);
    saveTerms([{ term: 'a', definition: 'A', domain: 'rv-default', importance: 0.5 }], null, null);
  ...
  it('领域开关打开 ⇒ reviewCount 等于 count（该域全部跟随）', () => {
    ...
    getDb().prepare(`UPDATE term_domain SET review_enabled = 1 WHERE name = 'rv-on' AND owner_id = ''`).run();
  ...
  it('★ 部分纳入：开关开着但词条被逐条反选 ⇒ reviewEnabled=true 而 reviewCount < count', () => {
    ...
    getDb().prepare(`UPDATE term_domain SET review_enabled = 1 WHERE name = 'rv-part' AND owner_id = ''`).run();
  ...
  it('★ 孤儿域（登记册里没有的域）恒 reviewEnabled=false，但 reviewCount 仍如实算', () => {
    saveTerms([{ term: 'e1', definition: '1', domain: 'rv-orphan', importance: 0.5 }], null, null);
    getDb().prepare(`DELETE FROM term_domain WHERE name = 'rv-orphan' AND owner_id = ''`).run(); // 造孤儿态
    scopeTerm('e1', true);
