...
> 版本：v1.11 | 状态：[活跃] | 更新：2026-09-25（⚰️ **本契约在五张表上失去了对象**：学习流/知识图功能 2026-09-25 整族下线（批次 K），**迁移 v44 把 `flow_def`/`flow_run`/`flow_run_step`/`knowledge_node`/`knowledge_edge` 整族 DROP** ⇒ §8.2 里 M2d-3 为这五张表做的 `owner_id` 加列（迁移 v33）连同"收口 A 的图谱 B 能看见"那个旧洞，**一起随宿主消失**。这不是"未做"，是"没东西可管了"；活着的 M2d-3 那半边（`quiz_bank`/`quiz_stats`/`quiz_notes` 三列）一字未动。按 §0.4 **回标不回改**：§8.2 与版本沿革原文**一字未动**，只在那两行行尾加 ⚰️ 标注。判决原话与理由见 `docs/STUDY-FLOW-SPEC.md` 墓碑与 `CHANGELOG.md` 2026-09-25 本批行。）
