...
| `packages/web/src/features/chat/DocModeControl.tsx` | 文档模式控件（挂在 composer 上方）：三态按钮 / 已载入 pill（名·字数·截断标·清除）/ 展开面板可粘贴或选 `.txt,.md,.markdown` 文件；刷新后只凭 `GET /api/doc` 的元信息复原。切换会话时 `useEffect([sessionId])` 重取（`alive` 标志防串台）。**→ 2026-09-07 文案修正三处**：按钮 `title` 按「有无资料」分两态（有资料时直说会替换哪一份）、pill 里扩展名单独成 span（截断时 `.txt` 不丢）、面板顶部补一行机制说明（阈值与会替换原先只在 tooltip 里）——详见 `docs/DOC-RAG-SPEC.md` §4.1。**→ 2026-09-07 打磨两项**：清除按钮 chip 化（可点性）、名与扩展名贴回一体（gap 不再插进文件名中间），after 图证 `shots/after/` |
