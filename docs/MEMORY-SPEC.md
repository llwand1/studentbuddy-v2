...
> 血缘：机制照搬 `@earendil-works/pi-coding-agent` 的 **Compaction**（实读 `ai-orchestrator/node_modules/@earendil-works/pi-coding-agent/docs/compaction.md`）+ **AGENTS.md 上下文文件**（实读同目录 `examples/sdk/07-context-files.ts`）。**不新造记忆范式、不引向量库、不引新依赖。**
