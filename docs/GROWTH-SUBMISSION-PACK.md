...
> 版本：v0.1.0 | 状态：[活跃] | 更新：2026-09-24 00:3x（批次 G-2 新建）
...
| ⚠️ 我们**没有**的 | 隐私政策页、服务条款页、状态页、邮件列表、移动端原生 app | 实测 `https://11wand.com/privacy`、`/terms-of-service`、`/legal` **全部返回 2548 字节 SPA 壳**（不是内容页，09-24 现读） | 高 |
...
| # | 可粘贴正文 | 字数 | 侧重 |
|---|---|---|---|
| **A（建议）** | StudentBuddy：把学过的东西抽成你自己的词条库，词条自己长出复习排期、出题判分、概念图和每日总结。开源、可自托管，也可以直接用我们上线的那一份。 | 77 | 差异化＝**词条是主体**（09-22 老板点单的主轴） |
| B | 自托管的 AI 学习助手：学→练→析→忆→反馈一条闭环自己转，用你自己的模型 Key，数据留在你自己的服务器上。 | 56 | 差异化＝**自托管＋自带 Key**（自托管导航站吃这一口） |
| C | 不是一个套了学习提示词的聊天框。它以你自己的词条库为中心：一个词进了库，出题、复习、概念图、每日总结都围着它转。 | 56 | 差异化＝**跟「聊天框」划清**（中文社区向） |
...
> studentbuddy 是一个开源、可自托管的 AI 学习助手。它把「学 → 练 → 析 → 忆 → 反馈」做成一条自动运转的闭环：你只管提问，讲解、出题、判分、复习排期、每日总结自己往下走。核心是一份**你自己的词条库**——对话里学到的概念自动入库，一个词进了库就会驱动出题、决定复习时机（1/2/4/7/15/30/60 天）、连进概念图谱、沉淀进每日总结、还能拿去对战。用你自己的模型 Key，数据存在你自己的服务器（一个 SQLite 单文件，备份＝拷一个目录）。MIT 开源，也可免注册直接体验。

**出处**：`landing-data.ts` 的 `INTRO.def` ＋ `TERM_SPINE` 五条 ＋ `PRIVACY_ITEMS` 第一条；逐条对得上 README「功能总览」。
...
>
> The loop runs on its own: **learn → practice → analyze → remember → feedback**. You ask questions; explaining, quiz-making, grading, review scheduling and daily summaries keep going by themselves.
>
...
> - it **weaves the concept graph** — new terms pulled from your follow-up questions link straight back to it
> - it **feeds the daily summary** and the study-trend card, whose numbers all come from SQL, never from the model
...
| 记忆／笔记 | Memory / Notes | 词条库 + 刷题笔记 + 跨会话画像 | 高 |
| 知识图谱 | Knowledge Graph | 知识图页（`STUDY-FLOW-SPEC.md`；★ 是**数据图**不是图库那类图，措辞别拔到 "graph database"） | 中 |
...
### 6.1 现成可用（6 张，全部真机截图、已提交在仓内）

| 文件 | 尺寸 | 内容 | 用于 |
|---|---|---|---|
| `docs/images/landing-hero.png` | 1440×900 | 落地页首屏（品牌牌 + 知识图演示窗） | 首图／封面（★ 但对外首图更推荐 `packages/web/public/og/home.png` 1200×630，清单站缩图友好） |
| `docs/images/app-chat.png` | 1440×900 | 应用壳·对话（八视图侧栏 + 督促胶囊） | **主截图第 1 张**（一眼看得出是产品） |
| `docs/images/app-study-flow.png` | 1440×1000 | 学习流编排（SVG 画布 + 步骤面板） | 主截图第 2 张（差异化：可编排） |
