...
| M6 PK 微信答题（AI 出题双人对战，契约 `docs/PK-SPEC.md` v0.1.0） | 🔶 2026-09-09 契约定稿（与老板 grill 钉死六项——8 分钟固定时长局／P0 纯选择题／出题成功 +1、答对 +2、答错 −1、超时 −1、怠慢 −1 可累计且**分数可为负**／**全部计时在服务端**防作弊／**正确答案永不下发**给答题方／P0 局域网双人不做公网）。**P0-1 ✅ 2026-09-12 完成**：登录（`pk_users` v10 + `/auth/login`、`/auth/me` + `UserAuthBox`，提交 `e02d67a`）＋房间（`pk/room.ts` 状态机 + `/rooms` 四端点）＋SSE `pk:` 频道与 `pk-state` 广播；测试 `pk-auth.test.ts` 11 例 + `pk-room.test.ts` 19 例，全量 44 文件 586 例全绿。**P0-2（计分/结算）与 P0-3（前端三页面）未开工**。★ 本行 **2026-09-12 补登**（此前 PK 整条线在本文档无索引，属 §0.11 漂移），**「M6」编号亦为本次新增，待老板确认**。瓶颈不在代码：P1 真微信卡在**资质**（个人主体申请不到已认证服务号）＋公网 HTTPS/域名前置 |
