...
![tests](https://img.shields.io/badge/tests-181%20files%20%2F%202495%20cases-brightgreen)
...
| **测试 + 机器强制门禁** | `npm run check` = tsc×3 + eslint + vitest + gates：单文件行数红线（server ≤400 / web ≤300）、禁 `any`、禁内联样式全部由脚本拦截，不靠自觉；交互层是**两层互补**——**14 个 `.test.tsx`**（jsdom 按文件 pragma 启用，锁交互逻辑）＋ **13 个真机探针脚本**（`tools/probes/*.mjs`，其中 9 个走 CDP 真点真渲染，锁 CSS 与真实浏览器行为） | 基线 **181 文件 / 2495 例**（2026-09-20 判断题批后 `metrics --tests` 实测口径；⚠️ 全量 vitest 另含并行批在途未提交改动且部分红，归属与披露见 [`docs/dev/test-plan.md`](docs/dev/test-plan.md) 顶注）；**逐文件不变量见 §3**（数字由 `node tools/metrics.mjs --tests` 产出，非手抄） |
...
│  ├─ routes/               REST 分域路由（20 个域文件；全仓 REST 注册 143 条）
