...
> ## ⚰️ 状态：**已下线（墓碑）** — 2026-09-25 整族删除，本文件不再描述现役能力
>
> ★ **一句话**：学习流与知识图**功能整体从仓里删掉了**（前端页面、路由、编排运行器、执行器注册、
>   chat 每轮的自动连边、shared 类型），数据库那七张表由**本仓第一条删除型迁移 v44** DROP。
>   本文件**保留全文**只作架构沿革（含 LangGraph/n8n/Dify 三方取材的那段判断，那部分内容与功能在否无关），
>   ⚠️ 但**文中任何"已落地/现役/待办"的字样都不再是事实**——引用前先读本段。
>
> **为什么删（老板原话，2026-09-25）**：「把学习流和知识图功能完全删除，因为这两个功能对于项目总体的
> 联动性太低了」；追问线上那半时拍板「也一起删了，现在用户太少了，没有必要留着，因为之后有大规划」
> ⇒ **不备份、不归档、不做只读归档页**（那条建议被我提出、被明确否掉，判决留在这里而不是藏在提交里）。
>
> **实证的动因（不是审美判断）**：`chat/post-turn.ts` 每一轮回复都在写 `knowledge_edge`，而知识图页
> 上线以来无人访问 ⇒ **写侧一直在付成本、读侧没有收益**，这就是"联动性太低"的可核对版本。
>
> **被这条牵连的三处契约/资产（同批改，别拿旧文当已交付）**：
>  · `docs/KNOWLEDGE-FOLLOWUP-SPEC.md`「向 AI 追问」**只剩 fork 那一半**——自动连边随本域一起删（老板原话需求的一半被撤销）
>  · `docs/TENANCY-SPEC.md` §8.2 的 M2d-3 行：`knowledge_node`/`knowledge_edge` 归主已做完又整族删除
>  · `docs/GROWTH-SUBMISSION-PACK.md`：对外素材里「知识图谱」术语行与 `docs/images/app-study-flow.png` 截图作废
>
> **回收路径（若将来"大规划"里要重做）**：架构判断留在本文件与 §1 三层架构，代码从 git 历史取
> （功能批＝`git log --oneline -- packages/web/src/features/study-flow`），数据**没有**回收路径（判决如此）。
>
> ---
>
> 版本：v1.0 | 状态：~~[活跃]~~ **已下线**（2026-09-16 立，v0.2.27 落地；2026-09-25 整体删除，见上方墓碑）
>
> ★ 契约先行（AGENTS.md 工程红线）本批**倒置**了：
